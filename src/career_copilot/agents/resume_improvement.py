...
def generate_full_resume(
    ...
    """
    Generate a full, updated resume document (no commentary) based on:
    - Original resume
    - Target job
    - RAG context
    - Prior chat turns (edits the user requested)
    """
    if not job:
        return "I don't have the job context. Please start from the job page again."
    ...
    messages: list[dict[str, str]] = [{"role": "system", "content": system}]
    for h in conversation_history:
        # For regeneration we only care about what the user asked to change
        # (\"shorten bullet 2\", \"add more ML pipeline experience\", etc.),
        # not the assistant's analytical responses.
        role = h.get("role")
        content = h.get("content") or ""
    ...
    messages.append(
        {
            "role": "user",
            "content": (
                "Now, using the original resume above and any concrete edits we discussed, "
                "write the full updated resume in your own clean default layout.\n\n"
                "Requirements:\n"
                "- Include only resume sections and bullet points (contact, summary, experience, projects, education, skills, etc.).\n"
                "- Do NOT include any commentary, analysis, ATS scores, or instructions.\n"
                "- Do NOT mention that this was generated by an AI.\n"
                "- Output plain text that could be pasted into a document editor.\n"
                "- Do NOT use Markdown formatting (no **bold**, no ``` fences, no # headings)."
            ),
        }
    )
