...
async def get_job_detail(
    ...
    set_job_feedback(conn, user_id, job_id, "ingested", "details_viewed")
    conn.commit()
    conn.close()
...
async def get_improve_resume(
    ...
    if not row:
        conn.close()
        return RedirectResponse(url="/recommendations", status_code=303)
    set_job_feedback(conn, user_id, job_id, "ingested", "resume_improvement_opened")
    conn.commit()
    conn.close()
...
async def get_prepare_interview(
    ...
    if not row:
        conn.close()
        return RedirectResponse(url="/recommendations", status_code=303)
    set_job_feedback(conn, user_id, job_id, "ingested", "interview_preparation_opened")
    conn.commit()
    conn.close()
    job = row_to_job_dict_snippet(row, description_max_chars=JOB_DESCRIPTION_SNIPPET_MAX_CHARS)
    return templates.TemplateResponse(
