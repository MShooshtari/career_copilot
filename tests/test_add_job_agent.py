...
def test_title_from_url_path_slug() -> None:
    assert (
        title_from_url_path("https://example.com/careers/Senior-Data-Scientist")
...
def test_title_from_url_path_uuid_skipped() -> None:
    assert (
        _title_from_url_path("https://rippling.com/jobs/e610fefb-7086-45de-a4a9-fd0c5e9fa4e4") == ""
    )
...
def test_indeed_title_from_query_with_q() -> None:
    assert (
        _indeed_title_from_query("https://ca.indeed.com/viewjob?jk=1&q=machine+learning+engineer")
...
def test_indeed_company_from_path_cmp() -> None:
    assert (
        _indeed_company_from_path("https://ca.indeed.com/cmp/Ind-Technology-Pty-Ltd/jobs?jk=1")
        == "Ind Technology Pty Ltd"
    )
...
def test_generic_title_indeed_with_q() -> None:
    assert (
        _generic_title_from_url("https://ca.indeed.com/viewjob?jk=1&q=data+engineer")
        == "Data Engineer"
    )
...
def test_generic_title_indeed_cmp_path() -> None:
    assert (
        _generic_title_from_url("https://ca.indeed.com/cmp/Acme-Corp/jobs?jk=1")
        == "Job at Acme Corp"
    )
...
def test_generic_title_bamboohr_subdomain() -> None:
    assert (
        _generic_title_from_url("https://tractionrec.bamboohr.com/careers/135")
        == "Job at Tractionrec"
    )
...
def test_company_from_bamboohr_subdomain() -> None:
    assert (
        _company_from_bamboohr_subdomain("https://tractionrec.bamboohr.com/careers/135")
        == "Tractionrec"
    )
...
def test_company_from_bamboohr_subdomain_with_hyphen() -> None:
    assert (
        _company_from_bamboohr_subdomain("https://my-company.bamboohr.com/careers") == "My Company"
    )
...
def test_extract_job_from_text_mocked_llm() -> None:
    with patch("career_copilot.agents.add_job._extract_with_llm") as mock_llm:
        ...
        out = extract_job_from_text(
            "We are hiring a Software Engineer at Acme Inc. Remote. $100k-150k."
        )
...
def test_extract_job_from_url_fallback_generic_title() -> None:
    with patch(
        "career_copilot.agents.add_job._fetch_page_html",
        return_value="<html><body>Loading...</body></html>",
    ):
        with patch("career_copilot.agents.add_job._extract_with_llm") as mock_llm:
            mock_llm.return_value = {
                "title": None,
